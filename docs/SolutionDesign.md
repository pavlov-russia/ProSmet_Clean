...
Связано: `Vision.md`, `Scope.md`, `docs/ConceptualDesign.md`, `docs/Architecture.md`, `docs/Domain/DeterministicEstimation.md`, `docs/Evals/QualityGates.md`, `docs/contracts/`, `docs/contracts/VisualIdentityAndMarketingSite.md`
...
- `docs/contracts/VisualIdentityAndMarketingSite.md` — visual direction, стартовая палитра, UI принципы и public marketing site boundary.
...
apps/marketing
  ...
  -> home / vertical landing / demo request / legal pages
  -> synthetic product screenshots and marketing copy
  -> marketing lead form with consent
  -> no direct tenant/customer data access
...
packages/ui
  ...
  -> visual tokens from VisualIdentityAndMarketingSite
  -> marketing site components
  -> admin/settings components
  -> dashboard components
  -> review components
  -> widget and client link components
...
flowchart LR
  Client["B2C client"]
  Buyer["B2B buyer"]
  MarketingSite["ProSmet Marketing Site"]
  MarketingLead["Marketing Lead API"]
...
### 5.4. ProSmet marketing context

Используется публичным сайтом ProSmet для рекламы сервиса потолочным компаниям.

Особенности:

- не является tenant runtime и не обслуживает клиентские сметы;
- не имеет прямого доступа к tenant/customer data, price books, calculations or client links;
- показывает только публичные материалы, synthetic screenshots and demo data;
- demo/pilot request собирает phone/email/name только после consent;
- marketing payload не отправляется в LLM;
- production deploy, external analytics and external forms require explicit architect approval.

## 6. API Surface

### 6.0. Marketing site API

- `POST /api/marketing/leads` — создать demo/pilot request after marketing consent.
- `POST /api/marketing/events` — записать неперсональное событие сайта: page view, CTA, demo interest.

Ограничения:

- marketing lead не является tenant lead;
- данные marketing lead не передаются в AI gateway;
- реальные отзывы, сметы, прайсы and customer screenshots нельзя использовать без отдельного approval;
- external CRM/analytics integration is out of scope until ArchitectInterventionRequest.
